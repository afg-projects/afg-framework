package io.github.afgprojects.framework.security.auth.permission.adapter;

import io.github.afgprojects.framework.data.core.DataManager;
import io.github.afgprojects.framework.security.auth.permission.entity.SecCasbinRule;
...
public class JdbcCasbinAdapter implements BatchAdapter {
    ...
    public void loadPolicy(@NonNull Model model) {
        try {
            ...
            // 表可能还未创建（Liquibase 尚未执行），忽略错误
            log.debug("Casbin rules table not available yet, skipping policy load: {}", e.getMessage());
        }
    }
