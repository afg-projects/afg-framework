...
public class MinioFileStorage implements FileStorage {
     ...
     * @param presigner 预签名器（可选）
     */
    public MinioFileStorage(@NonNull String bucket, @NonNull S3Client s3Client, @Nullable S3Presigner presigner) {
        this.bucket = bucket;
        this.s3Client = s3Client;
        this.presigner = presigner;

        log.info("MinIO storage initialized: bucket={}", bucket);
    }
    ...
    public DownloadResult download(@NonNull String key) {
        try {
            ...
            GetObjectResponse response = s3Client.getObject(getRequest).response();
