package io.github.afgprojects.framework.ai.llm.output;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.JsonMappingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import io.github.afgprojects.framework.ai.core.output.OutputParsingException;
import io.github.afgprojects.framework.ai.core.output.StructuredOutputParser;
import org.jspecify.annotations.NonNull;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.util.regex.Matcher;
import java.util.regex.Pattern;

/**
 * JSON-based structured output parser.
 * <p>
 * This parser uses Jackson's ObjectMapper to parse JSON responses from LLMs
 * into Java objects. It handles common issues like:
 * <ul>
 *   <li>Markdown code blocks (```json ... ```)</li>
 *   <li>Extra whitespace and newlines</li>
 *   <li>JSON embedded in text responses</li>
 * </ul>
 *
 * <p>Example usage:
 * <pre>{@code
 * // Create a parser
 * JsonStructuredOutputParser<Person> parser =
 *     new JsonStructuredOutputParser<>(Person.class);
 *
 * // Get format instructions
 * String instructions = parser.getFormatInstructions();
 *
 * // Parse response
 * Person person = parser.parse(response.content());
 * }</pre>
 *
 * @param <T> the type of object to parse
 * @author AFG Projects
 * @since 1.0.0
 */
public class JsonStructuredOutputParser<T> implements StructuredOutputParser<T> {

    private static final Logger log = LoggerFactory.getLogger(JsonStructuredOutputParser.class);

    private static final Pattern MARKDOWN_CODE_BLOCK =
            Pattern.compile("```(?:json)?\\s*\\n?([\\s\\S]*?)\\n?```", Pattern.CASE_INSENSITIVE);

    private final Class<T> targetClass;
    private final ObjectMapper objectMapper;
    private final String formatInstructions;

    /**
     * Creates a new JsonStructuredOutputParser with a default ObjectMapper.
     *
     * @param targetClass the class to parse into
     */
    public JsonStructuredOutputParser(@NonNull Class<T> targetClass) {
        this(targetClass, new ObjectMapper());
    }

    /**
     * Creates a new JsonStructuredOutputParser with a custom ObjectMapper.
     *
     * @param targetClass the class to parse into
     * @param objectMapper the ObjectMapper to use for parsing
     */
    public JsonStructuredOutputParser(@NonNull Class<T> targetClass, @NonNull ObjectMapper objectMapper) {
        this.targetClass = targetClass;
        this.objectMapper = objectMapper;
        this.formatInstructions = generateFormatInstructions();
    }

    @Override
    @NonNull
    public T parse(@NonNull String content) {
        if (content == null) {
            throw new IllegalArgumentException("content must not be null");
        }
        log.debug("Parsing content for type: {}", targetClass.getSimpleName());

        // Clean and extract JSON
        String json = extractJson(content);

        if (json == null || json.isBlank()) {
            throw new OutputParsingException(
                    "No valid JSON found in response. Content: " + truncate(content, 200));
        }

        try {
            return objectMapper.readValue(json, targetClass);
        } catch (JsonMappingException e) {
            throw new OutputParsingException(
                    "Failed to map JSON to " + targetClass.getSimpleName() + ": " + e.getMessage(), e);
        } catch (JsonProcessingException e) {
            throw new OutputParsingException(
                    "Invalid JSON format: " + e.getMessage() + ". JSON: " + truncate(json, 200), e);
        }
    }

    @Override
    @NonNull
    public String getFormatInstructions() {
        return formatInstructions;
    }

    @Override
    @NonNull
    public Class<T> getTargetClass() {
        return targetClass;
    }

    /**
     * Extracts JSON from the content.
     * <p>
     * Handles:
     * <ul>
     *   <li>Markdown code blocks (```json ... ```)</li>
     *   <li>Raw JSON objects or arrays</li>
     *   <li>JSON embedded in text</li>
     * </ul>
     *
     * @param content the raw content
     * @return the extracted JSON string, or null if not found
     */
    private String extractJson(String content) {
        if (content == null || content.isBlank()) {
            return null;
        }

        String trimmed = content.trim();

        // Try to extract from markdown code block first
        Matcher markdownMatcher = MARKDOWN_CODE_BLOCK.matcher(trimmed);
        if (markdownMatcher.find()) {
            String extracted = markdownMatcher.group(1).trim();
            if (!extracted.isBlank()) {
                return extracted;
            }
        }

        // Check if the entire content looks like JSON
        if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
            return trimmed;
        }

        // Try to find JSON object in the content
        int start = trimmed.indexOf('{');
        int end = trimmed.lastIndexOf('}');
        if (start >= 0 && end > start) {
            return trimmed.substring(start, end + 1);
        }

        // Try to find JSON array
        start = trimmed.indexOf('[');
        end = trimmed.lastIndexOf(']');
        if (start >= 0 && end > start) {
            return trimmed.substring(start, end + 1);
        }

        return null;
    }

    /**
     * Generates format instructions for the target class.
     */
    private String generateFormatInstructions() {
        StringBuilder sb = new StringBuilder();
        sb.append("\n\nYour response must be a valid JSON object.\n\n");

        sb.append("Target type: ").append(targetClass.getSimpleName()).append("\n\n");

        sb.append("Important:\n");
        sb.append("- Return ONLY the JSON object, no additional text\n");
        sb.append("- Do not include markdown code blocks unless specifically asked\n");
        sb.append("- Ensure all required fields are present\n");
        sb.append("- Use proper JSON syntax with double quotes\n");
        sb.append("- Use null for missing optional fields\n");

        return sb.toString();
    }

    /**
     * Truncates a string for error messages.
     */
    private String truncate(String str, int maxLength) {
        if (str == null) {
            return "null";
        }
        if (str.length() <= maxLength) {
            return str;
        }
        return str.substring(0, maxLength) + "...";
    }
}
