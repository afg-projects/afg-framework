...
/**
 * AfgChatClient 默认实现 - 委托 Spring AI ChatClient
 *
 * @author afg-projects
 * @since 1.0.0
 */
public class DefaultAfgChatClient implements AfgChatClient {
    ...
    public DefaultAfgChatClient(@NonNull ChatClient chatClient) {
        this(chatClient, null, null, null);
    }

    private DefaultAfgChatClient(
    ...
    public @NonNull AfgChatClient withSystemPrompt(@Nullable String systemPrompt) {
        return new DefaultAfgChatClient(this.chatClient, systemPrompt, this.modelName, this.conversationId);
    }

    @Override
    public @NonNull AfgChatClient withModel(@Nullable String modelName) {
        return new DefaultAfgChatClient(this.chatClient, this.systemPrompt, modelName, this.conversationId);
    }

    @Override
    public @NonNull AfgChatClient withConversationId(@Nullable String conversationId) {
        return new DefaultAfgChatClient(this.chatClient, this.systemPrompt, this.modelName, conversationId);
    }
