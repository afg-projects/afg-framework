 ...
 * 技能执行器默认实现
 *
 * <p>使用 {@link AfgChatClient} 执行技能，支持工具调用。
 *
 * @author afg-projects
 * @since 1.0.0
 */
...
public class DefaultSkillExecutor implements SkillExecutor {
    ...
    public DefaultSkillExecutor(
        ...
        this(skillRegistry, chatClient, null);
    }
    ...
    public @NonNull SkillResult execute(@NonNull SkillContext context) {
        ...
        if (definitionOpt.isEmpty()) {
            return SkillResult.failure(context.getSkillName(), "Skill not found: " + context.getSkillName());
        }
    ...
    private @NonNull SkillResult executeDefinition(
        ...
        try {
            ...
            return SkillResult.success(definition.name(), content);

        } catch (Exception e) {
            log.error("Skill execution failed: {} - {}", definition.name(), e.getMessage());
            return SkillResult.failure(definition.name(), "Skill execution failed: " + e.getMessage());
        }
    }
