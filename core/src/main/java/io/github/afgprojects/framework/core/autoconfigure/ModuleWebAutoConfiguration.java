...
public class ModuleWebAutoConfiguration implements WebMvcConfigurer {
    ...
    /**
     * 检查类是否为 Controller 且未标记忽略模块前缀
     */
    private boolean isController(Class<?> clazz) {
        // 如果标记了忽略模块前缀注解，则返回 false，不添加前缀
        if (AnnotationUtils.findAnnotation(clazz, IgnoreModuleContextPath.class) != null) {
            return false;
        }
        return AnnotationUtils.findAnnotation(clazz, RestController.class) != null
                ...
                || (AnnotationUtils.findAnnotation(clazz, ResponseBody.class) != null && AnnotationUtils.findAnnotation(clazz, RequestMapping.class) != null);
    }
